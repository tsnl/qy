...
class BaseExp(BaseNode, metaclass=abc.ABCMeta):
    def __init__(self, loc: "feedback.ILoc"):
        super().__init__(loc)
        self.x_typer_outer_context = None
...
class PostfixVCallExp(BaseCallExp):
    def __init__(self, loc: "feedback.ILoc", called: "BaseExp", arg: "BaseExp"):
...
class ModuleExp(BaseExp):
    module_id_counter = 0

    def __init__(self, loc, import_specs: List["ImportLine"], elements: List["BaseElement"]):
        super().__init__(loc)

        self.unid = ModuleExp.generate_fresh_module_id()

        self.import_specs = import_specs
        import_sym_names = [it.as_name for it in self.import_specs]
        self.table = Table(
            loc, "module-exp", elements,
            import_sym_names=import_sym_names,
    ...
    def generate_fresh_module_id():
        ModuleExp.module_id_counter += 1
        return ModuleExp.module_id_counter


class ImportLine(BaseNode):
    def __init__(self, loc, as_name: str, from_string_exp: "StringExp", exposed_names: List[str]):
        super().__init__(loc)
        self.as_name = as_name
        self.from_string_exp = from_string_exp
        self.exposed_names = exposed_names


class ChainExp(BaseExp):
    def __init__(self, loc, elements, opt_tail: Optional[BaseExp]):
        super().__init__(loc)
...
class ConstructionExp(BaseExp):
    constructed_type_spec: "BaseTypeSpec"
    arg_exps: List["BaseExp"]

    def __init__(self, loc, constructed_type_spec, arg_exp):
        super().__init__(loc)
        self.constructed_type_spec = constructed_type_spec
...
class Table(object):
    ...
    def parse_typing_elem(self, elem: "BaseTypeElement"):
...
class TypingTypeIDElement(BaseTypingElement):
    interface_spec: "BaseTypeSpec"

    def __init__(self, loc, id_name, interface_spec_list, extension_elements):
        super().__init__(loc, id_name)
        self.interface_spec_list = interface_spec_list
        self.extension_table = Table(
            loc, "type-extension", extension_elements,
            accepts_binding_elements=True,
            accepts_typing_elements=True,
            accepts_imperative_elements=False,
            import_sym_names=["Self"]
        )
...
class GetMethodCallByNameExp(BaseExp):
    container: BaseExp
    method_name: str

    def __init__(self, loc, container, method_name):
        super().__init__(loc)
        self.container = container
